...
Cambios v1.1:
  · _read_state() ahora reintenta hasta 3 veces con pausa de 2ms si lee
    archivo vacío o JSON malformado (fix race condition: Lua trunca el archivo
    al abrirlo en modo "w" justo cuando Python lo lee → 0 bytes temporales).
  · soft_reset() ahora exige "in_combat"=True además de HP >= MIN_HP_VALID
    para evitar que un P1_HP=255 (valor de muerte en SF2CE) engañe al bridge.
  · Añadido log de errores de lectura con contador (no satura consola).
"""
...
# ── RUTAS POR DEFECTO ─────────────────────────────────────────────────────────
BASE_DIR   = r"C:\proyectos\MAME"
MAME_EXE   = r"C:\mame\mame64.exe"
MAME_ROM   = "sf2ce"
MAME_LUASC = r"C:\proyectos\MAME\lua\autoplay_bridge.lua"

MIN_HP_VALID = 100   # HP mínima para considerar combate válido

# Máximo de reintentos de lectura de state.txt antes de devolver None
_READ_RETRIES = 3
_READ_RETRY_DELAY = 0.002   # 2ms entre reintentos
...
class MAMEBridge:
    ...
    def __init__(self, instance_id: int = 0, base_dir: str = BASE_DIR):
        ...
        # Contadores de diagnóstico (no saturan consola)
        self._read_fail_count  = 0
        self._read_ok_count    = 0
        self._last_fail_report = 0
    ...
    def _read_state(self) -> Optional[Dict]:
        """
        Lee state.txt y devuelve el dict parseado.

        [FIX v1.1] Reintenta hasta _READ_RETRIES veces si el archivo está
        vacío o el JSON es inválido. Esto resuelve la race condition:
        Lua abre state.txt en modo "w" (trunca a 0 bytes) y Python puede
        leerlo justo en ese instante antes de que Lua termine de escribir.
        Un reintento de 2ms es suficiente para que Lua complete la escritura.
        """
        for attempt in range(_READ_RETRIES):
            try:
                with open(self._state_file, "r", encoding="utf-8") as f:
                ...
                if not raw:
                    # Archivo vacío: Lua está en mitad de una escritura
                    if attempt < _READ_RETRIES - 1:
                        time.sleep(_READ_RETRY_DELAY)
                        continue
                    self._record_fail("empty")
                ...
                # state.txt aún no existe (MAME justo arrancó)
                return None
            except (json.JSONDecodeError, ValueError):
                # JSON parcial: Lua escribió a medias
                if attempt < _READ_RETRIES - 1:
                    time.sleep(_READ_RETRY_DELAY)
                    continue
                self._record_fail("json_error")
    ...
    def _record_fail(self, reason: str):
        """Registra un fallo de lectura; solo imprime cada 100 fallos."""
        self._read_fail_count += 1
        total = self._read_fail_count + self._read_ok_count
        if self._read_fail_count - self._last_fail_report >= 100:
            self._last_fail_report = self._read_fail_count
    ...
    def step(self, buttons: List[int]) -> Optional[Dict]:
        """
        Envía inputs a MAME y devuelve el estado del siguiente frame.

        El dict incluye (entre otras claves):
          p1_hp, p2_hp, p1_x, p2_x, p1_dir, p1_char, p2_char, timer,
          p2_crouch, p2_anim, p2_y_vel, p2_airborne,
          boom_active, boom_x_est, boom_incoming, boom_slot_active,
          fk_landing_this_frame, frame,
          in_combat  ← True solo cuando sm_state=="IN_COMBAT" en Lua (v1.5+)
        """
        self._write_input(buttons)
    ...
    def soft_reset(self, timeout: float = 90.0) -> bool:
        """
        Espera a que MAME esté en un combate válido.

        [FIX v1.1] Además de HP >= MIN_HP_VALID, ahora exige in_combat=True
        (campo añadido en autoplay_bridge.lua v1.5). Esto evita que:
          · P1_HP=255 (valor post-muerte en SF2CE) sea confundido con HP válido.
          · Un estado de menú con HP residual pase como "combate listo".

        Si state.txt no tiene "in_combat" (Lua antiguo), se usa la lógica HP
        ...
        while time.time() < deadline:
            ...
            if st:
                ...
                # in_combat está disponible desde autoplay_bridge.lua v1.5
                if "in_combat" in st:
                    if st["in_combat"] and p1 >= MIN_HP_VALID and p2 >= MIN_HP_VALID:
                        try:
                            os.remove(self._reset_file)
    ...
    def launch_mame(self, mame_exe: str = MAME_EXE,
        ...
        cmd = [
            ...
            "-nothrottle" if self.instance_id > 0 else "",
        ]
...
>>>>>>> 487d32d03f00355dc78bbb22fb4d226e2cb07f0e
            pass